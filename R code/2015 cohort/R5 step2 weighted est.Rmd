...
our_data <- read.dta13(paste0(mainDir,"/MRP/2015_cohort/data/our_data_FULL.dta"))
nrow(our_data)
...
survey_design_all <- svydesign(
  data = our_data, 
  strata = ~w5varstrat,     #indicates the stratum from which the PSUs were drawn
  ids = ~w5varunit,         #indicates Primary Sampling Unit (PSU) or cluster
  nest = TRUE, 
  weights = ~w5anfinwgt0
)
```
...
```{r,echo=F}
saveRDS(est.1, file=paste0(mainDir,"/MRP/2015_cohort/data/results.1.RData"))
saveRDS(est.2, file=paste0(mainDir,"/MRP/2015_cohort/data/results.2.RData"))
```
