...
Round_5_data$frailty <- Round_5_data$frailty+1
```
...
```{r,echo=F, results='hide'}
out.data <- Round_5_data[,c("division", "age", "race", "frailty")]
...
P1 <- length(unique(Round_5_data$age)) #5 levels
P2 <- length(unique(Round_5_data$race)) #3 levels
D <- length(unique(Round_5_data$division)) #9 levels
...
saveRDS(stanfit, file=paste0(mainDir,"/MRP/2015_cohort/data/stanfit_v6.RData"))

summary <- summary(stanfit)
saveRDS(summary, file=paste0(mainDir,"/MRP/2015_cohort/data/summary_v6.RData"))

post_samples <- rstan::extract(stanfit,
                         pars=c("b0","b_age","b_race","a"))
saveRDS(post_samples, file=paste0(mainDir,"/MRP/2015_cohort/data/post_samples_v6.RData"))
```
...
```{r,echo=F}
post_samples <- readRDS(paste0(mainDir,"/MRP/2015_cohort/data/post_samples_v6.RData"))
```
...
tmp.b0 <- post_samples[["b0"]][which_iter,1,]
tmp.b_age <- post_samples[["b_age"]][which_iter,,]
tmp.b_race <- post_samples[["b_race"]][which_iter,,] 
tmp.a <- post_samples[["a"]][which_iter,,] 

tmp.1 <- as.matrix(pop.data.dum[which_cell, 5:9])%*%as.matrix(tmp.b_age)
...
tmp.eta <- tmp.b0 + tmp.1 + tmp.2 + tmp.3
...
```{r}  
saveRDS(pop_est, file=paste0(mainDir,"/MRP/2015_cohort/data/results.3.v6.RData"))
```
