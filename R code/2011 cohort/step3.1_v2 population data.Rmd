...
store_alldata <- read.csv(paste0(mainDir,"/MRP/pop_data/ACS_county_pop_est/cc-est2011-alldata.csv"))

# Select only 2011 population estimate
# 4 = 7/1/2011 population estimate
county_data <- store_alldata[store_alldata$YEAR==4,]
...
pop.data <- data.frame("fips" = rep(fips, each=15),
                       "age_label" = rep(rep(c("65-69", "70-74", "75-79", "80-84", "85+"), each = 3), 3143),
                       "age" = rep(rep(1:5, each = 3), 3143),
                       "race_label" = rep(rep(c("Non-Hispanic White/other", "Non-Hispanic Black", "Hispanic"),5),3143),
                       "race" = rep(rep(c(1:3),5),3143))
pop.data$N <- NA
...
```{r}
saveRDS(pop.data_, file=paste0(mainDir,"/MRP/pop_data/pop.data_v2.RData"))
```
